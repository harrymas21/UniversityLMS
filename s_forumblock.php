...
<div class="wrap">
  ...
  <?php
  $sql = "select * from students where user_id = $_SESSION[uid]";
  $rows = ExecuteQuery($sql);
  while ($row = mysql_fetch_array($rows)){
	  $ques = "select * from user where user_id = $row[blocked_by]";
	  $que = ExecuteQuery($ques);
	  $qu = mysql_fetch_array($que);
	  echo "<p class='fans'>You have been blocked from using this forum please contact teacher [$qu[fullname]]</p>";
	  }
  ?>
