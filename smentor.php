...
<div class="wrap">
  ...
  <div class="container">
   ...
   <?php
	...
	{
	echo "<br><h2 class=head1> No Mentors Available. </h2>";
	exit;
	}
	{
	while ($row= mysql_fetch_array($rows))
	{
		// use the info to get mentors profile pic
		$res=ExecuteQuery("select * from user where user_id='$row[user_id]'");
		$re=mysql_fetch_array($res);
		echo"<img src='$re[uimg]' style='height:80px; width:80px;'/> <b>$re[fullname]</b><br>
		<b>FIELD: </b>$row[field]<br>
		<b>ORGANIZATION:  </b>$row[organization]
		<br>$row[bio]<br><BR>
		<a href=smessage.php?id=$re[user_id] style='font-weight:bold;'><input type='button' value='Send Message'></a>
